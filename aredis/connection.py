...
from ._compat import (b, BytesIO, nativestr, basestring, iteritems,
...
class SocketBuffer(object):
    ...
    async def _read_from_socket(self, length=None):
        ...
        try:
            while True:
                data = await recv(self._sock, socket_read_size)
...
class PythonParser(BaseParser):
    ...
    def __init__(self, socket_read_size, kernel):
        self.socket_read_size = socket_read_size
        self._sock = None
        self._buffer = None
        self._kernel = kernel
...
class HiredisParser(BaseParser):
    ...
    async def read_response(self):
        ...
        while response is False:
            try:
                if HIREDIS_USE_BYTE_BUFFER:
                    bufflen = await recv_into(self._sock, self._buffer)
                    if bufflen == 0:
                        raise socket.error(SERVER_CLOSED_CONNECTION_ERROR)
                else:
                    buffer = await recv(self._sock, socket_read_size)
                    # an empty string indicates the server shutdown the socket
...
class Connection(object):
    ...
    def __init__(self, host='localhost', port=6379, db=0, password=None,
                 ...
                 parser_class=DefaultParser, socket_read_size=65536, kernel=None):
        self.pid = os.getpid()
        self.host = host
        self.port = int(port)
        self.db = db
        ...
        self._sock = None
        self._kernel = kernel or curio.Kernel()
    ...
    def __del__(self):
        self._kernel.run(self.disconnect(), shutdown=True)
    ...
    async def _connect(self):
        ...
        # socket.connect()
        err = None
        for res in await socket.getaddrinfo(self.host, self.port, 0,
                                            socket.SOCK_STREAM):
            ...
            try:
                ...
                curio.timeout_after(self.socket_connect_timeout,
                                    await sock.connect(socket_address))
                if self.socket_timeout:
                    curio.timeout_after(self.socket_timeout,
                                        await self.disconnect())
                return sock
    ...
    async def disconnect(self):
        ...
        if self._sock is None:
            return
        try:
            await self._sock.shutdown(socket.SHUT_RDWR)
