...
class TestScripting:
    ...
    async def test_eval(self, redis_cluster):
        ...
        # 2 * 3 == 6
        assert await redis_cluster.eval(multiply_script, 1, "a", 3) == 6
    ...
    async def test_eval_same_slot(self, redis_cluster):
        ...
        return value * value2
        """
        result = await redis_cluster.eval(script, 2, "A{foo}", "B{foo}")
        assert result == 8
    ...
    async def test_eval_crossslot(self, redis_cluster):
        ...
        with pytest.raises(RedisClusterException):
            await redis_cluster.eval(script, 2, "A{foo}", "B{bar}")
    ...
    async def test_evalsha(self, redis_cluster):
        ...
        # 2 * 3 == 6
        assert await redis_cluster.evalsha(sha, 1, "a", 3) == 6
    ...
    async def test_evalsha_script_not_loaded(self, redis_cluster):
        ...
        with pytest.raises(NoScriptError):
            await redis_cluster.evalsha(sha, 1, "a", 3)
