...
class TestRedisCommands:
    ...
    async def test_bitfield_overflow(self, r):
        ...
        assert [-128] == await r.bitfield(key).overflow("WRAP").incrby(
            "i8", 0, 128
        ).exc()
        await r.delete(key)
        assert [127] == await r.bitfield(key).overflow("SAT").incrby("i8", 0, 128).exc()
        await r.delete(key)
        assert [None] == await r.bitfield(key).overflow("fail").incrby(
            "i8", 0, 128
        ).exc()
    ...
    async def test_georadius_units(self, r):
        ...
        await r.geoadd("barcelona", *values)
        assert await r.georadius("barcelona", 2.191, 41.433, 1, unit="km") == ["place1"]
